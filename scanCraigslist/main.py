...
def scan_handler(event, context):
    ...
    finalListings = [];
    print(f"Must Include in Title {event['must_include_title']}");
    if event['must_include_title']:
        words = event['search_query'].split(' ');
        for listing in listings:
            isOkay = True;
            for word in words:
                if word.lower() in listing['title'].lower():
                    print("good");
                else:
                    isOkay = False;
            if isOkay:
                finalListings.append(listings);


    total_time_took = time.monotonic() - started_at
    print(f"Prev Results: {listings}")
    print(f"Final Results: {finalListings}")
    print(f"Pre Amount of Results: {len(results)}")
    print(f"Final Amount of Results: {len(finalListings)}")
...
if __name__ == "__main__":
    scan_handler({"amount_of_lists": 100,"category":"mca", "search_query":"1975 cb550", "must_include_title":True}, {})